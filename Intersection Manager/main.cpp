...
string dec2bin(unsigned short n){
    if (n == 31){return string("0000");}
    string binary = std::bitset<4>(n).to_string();
    return binary;
}
  
// show binary schedule
void ShowBin(vector<vector<Car>> schedule){
    for (auto c:schedule){for (auto a:c){cout<<dec2bin(a.dir)<<"\t";}cout<<endl;}cout<<endl; 
}

// show original schedule
void Show(vector<vector<Car>> schedule){
    for (auto c:schedule){for (auto a:c){
        if (a.dir == 31){cout<<"0"<<"\t";}
        else {cout<<(a.dir)<<"\t";}
    }cout<<endl;}cout<<endl; 
}
...
// show indices of boxes of intersection used of each Cars
void ShowFour(vector<vector<Car>> schedule){
    for (auto vec:schedule){for (auto v:vec){
        cout<<asBoxIndex(v.dir)<<"\t";
    }cout<<endl;} cout<<endl;
}

void ShowAll(vector<vector<Car>> schedule){
    ShowBin(schedule);    
    ShowFour(schedule);    
    Show(schedule);
}
...
vector<vector<int>> findGroup(vector<unsigned short> vec){
    ...
    if ( (vec[0]+vec[1]+vec[2]+vec[3])==15){choice.push_back(vector<int>{0,1,2,3}); }
    ...
    for (int i = 0; i<4; i++){if (vec[i]!=31){ choice.push_back(vector<int>{i}); }}
    return choice;
}


// check the correctness of function FindGroup() 
void checkFindGroup(){
    vector<unsigned short> N{2,6,14,31};
    vector<unsigned short> E{1,3,7,31};
    vector<unsigned short> S{8,9,11,31};
    vector<unsigned short> W{4,12,13,31};
    for (auto n:N){ for (auto e:E){ for (auto s:S){ for (auto w:W){
        cout<< "N, E, S, W = { "<< asBoxIndex(n)<< ", "<<asBoxIndex(e)<< ", "<<asBoxIndex(s)<< ", "<<asBoxIndex(w) <<" }"<<endl;
        vector<unsigned short> list{n,e,s,w};
        vector<vector<int>> vec = findGroup(list);
        for (auto a:vec){for (auto v:a){
            cout<<asBoxIndex(list[v])<<", ";
        }cout<<endl;}
        cout<<endl;
    }}}}
}
...
int main(int argc, char * argv[]){
    ...
    for(int i =0;i<4;i++){
        fscanf(file, "%s", from);//cout<<from[0]<<": ";
        for(int j = 0; j < len; j++){
            fscanf(file, " %s", str);   
    ...
    for (auto From:FromArr){for (auto To:ToArr){for (auto DistRate:DistRateArr){
        ...
        //for (auto c:rate){for (auto a:c){cout<<fixed<<setprecision(2)<<(a)<<"\t";}cout<<endl;}cout<<endl; 
        vector<vector<Car>> result;     int head[] = {0,0,0,0};    int round = 0;
        while( head[0]+head[1]+head[2]+head[3] < 4*len ){round += 1;
            //cout<<"round"<<round<<", "<<len<<endl;//
            //cout<<schedule[head[1]+3][1].dir<<","<<schedule[head[1]+3][1].arr<<endl;
            for( int i = 0; i < 4; i++){
                while(schedule[head[i]][i].dir==31 && schedule[head[i]][i].arr<round-1 && head[i]<len)
                    {head[i]+=1;} //cout<<head[i]<<", ";////
            }//cout<<endl;
